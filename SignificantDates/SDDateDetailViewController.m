...
- (void)viewDidLoad
{
    ...
    self.title = [self.managedObject valueForKey:@"accountName"];
    self.imageView.image = [UIImage imageWithData:[self.managedObject valueForKey:@"image"]];
    if ([self.managedObject isKindOfClass:[OrderHeader class]]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (dateType == SDDateHoliday) {
        ...
        if (indexPath.row == SDHolidayDateCell) {
            ...
            cell.detailTextLabel.text = [dateFormatter stringFromDate:newOrder.date];
        } else if (indexPath.row == SDHolidayDetailsCell) {
            cell.textLabel.text = @"Details";
            cell.detailTextLabel.text = newOrder.orderDesc;
        } else if (indexPath.row == SDHolidayObservedByCell) {
            cell.textLabel.text = @"Observed By";
            cell.detailTextLabel.text = [newOrder.observedBy componentsJoinedByString:@", "];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == SDHolidayObservedByCell) {
